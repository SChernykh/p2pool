...
TEST(wallet, input_output)
{
	...
	// Invalid pubkeys
	{
		...
		ASSERT_TRUE(w.assign(valid_spend_pub, valid_view_pub, NetworkType::Mainnet));
		ASSERT_FALSE(w.assign(invalid_spend_pub, valid_view_pub, NetworkType::Mainnet));
		ASSERT_FALSE(w.assign(valid_spend_pub, invalid_view_pub, NetworkType::Mainnet));
		ASSERT_FALSE(w.assign(invalid_spend_pub, invalid_view_pub, NetworkType::Mainnet));
	}
	...
	// if and only if K_s does, so these must never make it into a share.
	{
		...
		ASSERT_FALSE(w.assign(torsioned_spend_pub, valid_view_pub, NetworkType::Mainnet));
		ASSERT_FALSE(w.assign(valid_spend_pub, torsioned_view_pub, NetworkType::Mainnet));
		ASSERT_FALSE(w.assign(torsioned_spend_pub, torsioned_view_pub, NetworkType::Mainnet));
		...
		for (const hash& k : { identity_pub, order2_pub, order8_pub }) {
			ASSERT_FALSE(w.assign(k, valid_view_pub, NetworkType::Mainnet));
			ASSERT_FALSE(w.assign(valid_spend_pub, k, NetworkType::Mainnet));
		}
	...
	// three bad ones, so if it decodes then their base58 and checksums are good and torsion is what fails.
	{
		...
		ASSERT_TRUE(copy.assign(valid_spend_pub, valid_view_pub, NetworkType::Mainnet));
	...
	// Invalid prefix
	{
		...
		ASSERT_TRUE(w2.assign(w2.spend_public_key(), w2.view_public_key(), NetworkType::Invalid));

		ASSERT_FALSE(w1.valid());
		ASSERT_FALSE(w2.valid());
	}

	// Subaddress (mainnet, testnet, stagenet)
	{
		Wallet w1("86eQxzSW4AZfvsWRSop755WZUsog6L3x32NRZukeeShnS4mBGVpcqQhS6pCNxj44usPKNwesZ45ooHyjDku6nVZdT3Q9qrz");
		Wallet w2("BfPfAxrFBbLJiMzVGyNmgJbejtcZEMELsbWSv5h3xTc3LSvKpwJetPrNk4fMSZ4bYCA4o93yGdVqjUpZSqUGxWkZ5HwBzrM");
		Wallet w3("77jhPoqpvTnGqds2oAWGVmiZv5fiQb2E8BizTL3oh7ia54sQRSEQ7SAAyRj9TpvwgHGuX1cYNDCHP7WHJCBXHcLqSnmApfe");
	...
	auto check = [](NetworkType t, uint64_t prefix, uint32_t checksum, const char* address, const char* spendkey, const char* viewkey)
	{
		...
		// Test Wallet::assign()
		Wallet w2(nullptr);
		ASSERT_TRUE(w2.assign(w.spend_public_key(), w.view_public_key(), w.get_type()));
